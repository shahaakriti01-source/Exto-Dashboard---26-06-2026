...
function parseSurveyText(text) {
  ...
  for (let idx = 0; idx < rawLines.length; idx++) {
    ...
    // Otherwise: this is a new question title. (Heuristic: reasonable length, not a
    // pure number, not a stat fragment.)
...
async function extractSurveyText(file) {
  ...
  if (name.endsWith(".pdf")) {
    ...
    for (let p = 1; p <= pdf.numPages; p++) {
      ...
      // Reconstruct lines by grouping text items with similar vertical positions.
      const rows = {};
      ...
      Object.keys(rows).map(Number).sort((a, b) => b - a).forEach(y => {
        const lineStr = rows[y].sort((a, b) => a.x - b.x).map(i => i.s).join(" ").replace(/\s+/g, " ").trim();
...
function Dashboard({ onboardingEvents, payments, dataSource, lastUpdated, onRefresh, refreshing }) {
  ...
  const auditedTransactions = useMemo(() => {
    const merchantNameSet = new Set(merchantNames);
    return auditTransactions(incPayments, merchantNameSet);
  }, [incPayments, merchantNames]);
      ...
      <div style={{ padding: "24px 32px", maxWidth: 1440, margin: "0 auto" }}>
        ...
        {tab === "audit" && (
          <div>
            ...
            <div style={{ color: C.textFaint, fontSize: 12.5, marginBottom: 16 }}>
              Every payment transaction (for the Incentives tab's current phase/date range), showing whether it counts toward incentive and why.
            </div>
